...
You can use the `assertSaved` method to assert that a PDF was saved with specific properties. You should pass it a callable which will received an instance of `Jamespj\LaravelPdf\PdfBuilder`. If the callable returns `true`, the assertion will pass.
